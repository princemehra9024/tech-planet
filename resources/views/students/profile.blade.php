            ...
            <div class="pt-20">
                <div class="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-4">
                    <div>
                        <div class="flex items-center gap-3 flex-wrap">
                            ...
                            @if($user->role !== 'student' && $user->role)
                                @if($user->role === 'admin')
                                    <span class="inline-flex items-center gap-1.5 bg-amber-500/20 border border-amber-500/30 text-amber-400 font-extrabold uppercase tracking-wider text-xs px-3 py-1 rounded-full shadow-lg shadow-amber-500/10"><i class="fas fa-shield-alt"></i> Administrator</span>
                                @elseif($user->role === 'president')
                                    <span class="inline-flex items-center gap-1.5 bg-purple-500/20 border border-purple-500/30 text-purple-400 font-extrabold uppercase tracking-wider text-xs px-3 py-1 rounded-full shadow-lg shadow-purple-500/10"><i class="fas fa-crown"></i> Club President</span>
                                @elseif($user->role === 'secretary')
                                    <span class="inline-flex items-center gap-1.5 bg-cyan-500/20 border border-cyan-500/30 text-cyan-400 font-extrabold uppercase tracking-wider text-xs px-3 py-1 rounded-full shadow-lg shadow-cyan-500/10"><i class="fas fa-signature"></i> Secretary</span>
                                @elseif($user->role === 'treasurer')
                                    <span class="inline-flex items-center gap-1.5 bg-emerald-500/20 border border-emerald-500/30 text-emerald-400 font-extrabold uppercase tracking-wider text-xs px-3 py-1 rounded-full shadow-lg shadow-emerald-500/10"><i class="fas fa-coins"></i> Treasurer</span>
                                @elseif($user->role === 'media_manager')
                                    <span class="inline-flex items-center gap-1.5 bg-rose-500/20 border border-rose-500/30 text-rose-400 font-extrabold uppercase tracking-wider text-xs px-3 py-1 rounded-full shadow-lg shadow-rose-500/10"><i class="fas fa-photo-video"></i> Media Manager</span>
                                @endif
                            @endif
                        </div>
                        <div class="flex flex-wrap gap-4 mt-2 text-xs font-semibold text-slate-500">
                            @if($user->course)
                    ...
                    <div class="flex gap-2">
                        <span class="bg-purple-500/15 text-purple-700 dark:text-purple-300 border border-purple-500/30 px-4 py-1.5 rounded-full text-xs font-bold shadow-sm">ðŸ† {{ $user->xp }} XP</span>
                        <span class="bg-cyan-900 text-cyan-300 border border-cyan-800/35 px-4 py-1.5 rounded-full text-xs font-bold shadow-sm">Level {{ floor($user->xp / 100) }}</span>
                    </div>
                </div>
                
                <div class="mt-6 space-y-2">
                    <h3 class="text-[10px] uppercase font-bold tracking-widest text-muted">Developer Bio</h3>
                    <p class="text-charcoal/90 text-xs sm:text-sm leading-relaxed max-w-xl">{{ $user->bio ?? 'No biography added yet. Share some details about your technology interests!' }}</p>
                </div>
                    ...
                    <div class="mt-8 pt-6 border-t border-charcoal/5 ">
                        ...
                        <div class="flex flex-wrap gap-2.5">
                            @foreach($badges as $badge)
                                <span class="glass-card/70 border border-charcoal/5 text-charcoal/90 px-4 py-1.5 rounded-xl text-xs font-semibold shadow-inner flex items-center gap-1.5">
