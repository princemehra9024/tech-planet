...
@section('content')
<div class="space-y-8">
    <div class="relative overflow-hidden p-6 sm:p-8 rounded-3xl bg-gradient-to-r from-cyan-950/40 via-blue-950/40 to-purple-950/40 border border-charcoal/5 shadow-2xl">
        <div class="absolute -right-16 -top-16 w-32 h-32 bg-cyan-500/10 rounded-full blur-xl pointer-events-none"></div>
        <div class="relative z-10">
            <h2 class="text-3xl font-extrabold text-charcoal font-display flex items-center gap-2.5"><i class="fas fa-bolt text-cyan-400"></i> Assessment Arena</h2>
            <p class="text-muted mt-2 text-xs sm:text-sm">Validate your conceptual developer understanding, capture XP rewards, and level up.</p>
        </div>
    </div>
    ...
    <div class="grid md:grid-cols-2 gap-6">
        ...
        @endphp
        <div class="glass-card rounded-3xl p-6 border border-charcoal/5 hover:border-cyan-500/30 hover:shadow-cyan-500/5 transition-all duration-300 flex flex-col justify-between group relative overflow-hidden">
            ...
            <div>
                ...
                <div class="flex justify-between items-center mb-4">
                    ...
                    <span class="text-[9px] font-mono font-bold text-muted uppercase tracking-widest bg-charcoal/5 px-2 py-0.5 rounded-md">{{ $xpVal }}</span>
                </div>
                <h3 class="text-lg font-bold text-charcoal font-display mt-2 group-hover:text-cyan-400 transition-colors">{{ $quiz->title }}</h3>
                <p class="text-muted text-xs mt-2.5 leading-relaxed">{{ $quiz->description ?? 'Demonstrate syntax proficiency across selected engineering models.' }}</p>
                <div class="mt-4 flex items-center gap-2 text-[10px] font-bold text-muted uppercase tracking-wider">
            ...
            <div class="mt-6 pt-4 border-t border-charcoal/5 ">
                ...
                @else
                    <a href="{{ route('student.quiz.show', $quiz->id) }}" class="inline-block w-full text-center bg-gradient-to-r from-cyan-500 via-blue-500 to-purple-650 text-charcoal font-bold py-3 rounded-xl text-xs shadow hover:shadow-lg transition">Initialize Challenge</a>
                @endif
            </div>
        </div>
        @empty
        <div class="col-span-2 glass-card rounded-3xl p-12 text-center text-muted border border-charcoal/5 ">
