    ...
    <div>
        ...
        @if($upcomingEvents->count())
            <div class="grid md:grid-cols-2 gap-6 mt-6">
                ...
                <div class="glass-card rounded-3xl p-6 border border-charcoal/5 hover:border-cyan-500/30 hover:shadow-cyan-500/5 transition-all duration-300 flex flex-col justify-between relative overflow-hidden group">
                    ...
                    <div>
                        <div class="flex justify-between items-center mb-3">
                            ...
                            <span class="text-[9px] font-mono font-bold text-muted uppercase tracking-widest bg-charcoal/5 px-2 py-0.5 rounded-md">+10 XP reward</span>
                        </div>
                        <h3 class="text-lg font-bold text-charcoal font-display group-hover:text-cyan-400 transition-colors mt-2">{{ $event->title }}</h3>
                        <p class="text-muted text-xs mt-2 leading-relaxed">{{ Str::limit($event->description, 100) }}</p>
                        <div class="mt-5 space-y-2.5 text-xs font-semibold text-muted">
    ...
    <div>
        ...
        @if($myRegistrations->count())
            <div class="grid md:grid-cols-2 gap-5 mt-6">
                ...
                <div class="glass-card/40 border border-charcoal/5 rounded-3xl p-5 hover:border-cyan-500/20 transition-all duration-300 relative overflow-hidden flex items-center justify-between">
                    <div>
        ...
        @else
            <p class="text-slate-505 mt-4 text-xs bg-cream-dark/30 border border-charcoal/5 rounded-2xl p-6 text-center uppercase tracking-wider font-semibold">You have not registered for any active sprints.</p>
        @endif
    </div>
</div>
    ...
    <div class="glass-card rounded-3xl w-full max-w-lg border border-charcoal/10 shadow-2xl relative glass-card overflow-hidden">
        ...
        <form method="POST" action="{{ route('events.suggest') }}" class="p-6 space-y-4">
            ...
            <div>
                ...
                <input type="text" name="title" required class="w-full bg-cream-dark/50 border border-charcoal/10 rounded-xl px-4 py-2.5 text-charcoal placeholder-slate-600 focus:outline-none focus:ring-1 focus:ring-purple-500 transition text-xs sm:text-sm" placeholder="e.g. Next.js & Server Actions Workshop">
            </div>
            <div>
                <label class="block text-slate-350 font-semibold text-[10px] mb-1.5 uppercase tracking-widest">Description & Goals</label>
                <textarea name="description" required rows="4" class="w-full bg-cream-dark/50 border border-charcoal/10 rounded-xl px-4 py-2.5 text-charcoal placeholder-slate-600 focus:outline-none focus:ring-1 focus:ring-purple-500 transition text-xs sm:text-sm resize-none" placeholder="What should we learn? Who is the target audience?"></textarea>
            </div>
            <div>
            ...
            <div class="flex justify-end gap-3 pt-4 border-t border-charcoal/5 ">
                <button type="button" id="cancel-suggest-btn" class="bg-cream-dark/50 hover:bg-charcoal/5 border border-charcoal/10 text-charcoal/70 font-bold px-5 py-2.5 rounded-xl text-xs transition">
                    Cancel
                </button>
