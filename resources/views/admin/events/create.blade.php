...
@section('content')
<div class="mb-8">
    <a href="{{ route('admin.events.index') }}" class="text-muted hover:text-charcoal transition flex items-center gap-2 mb-4 text-sm">
        <i class="fas fa-arrow-left"></i> Back to Events
    </a>
    <h1 class="text-3xl font-display font-bold text-charcoal mb-2">Create New Event</h1>
    <p class="text-muted">Schedule and publish a new official club event.</p>
</div>
...
<div class="glass-card rounded-2xl p-6 md:p-8 max-w-3xl">
    <form method="POST" action="{{ route('admin.events.store') }}" class="space-y-6">
        ...
        <div>
            ...
            <input type="text" name="title" required value="{{ old('title') }}" placeholder="e.g. Intro to Web3 Workshop" class="w-full bg-cream-dark/50 /60 border border-charcoal/10 rounded-xl px-4 py-3 text-charcoal focus:outline-none focus:ring-1 focus:ring-purple-500 transition">
        ...
        <div>
            ...
            <textarea name="description" required rows="5" class="w-full bg-cream-dark/50 /60 border border-charcoal/10 rounded-xl px-4 py-3 text-charcoal focus:outline-none focus:ring-1 focus:ring-purple-500 transition resize-y" placeholder="Describe the event, agenda, requirements...">{{ old('description') }}</textarea>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <input type="datetime-local" name="date" required value="{{ old('date') }}" class="w-full bg-cream-dark/50 /60 border border-charcoal/10 rounded-xl px-4 py-3 text-charcoal focus:outline-none focus:ring-1 focus:ring-purple-500 transition">
        ...
        <div class="pt-4 border-t border-charcoal/5 flex justify-end gap-3">
            <a href="{{ route('admin.events.index') }}" class="px-5 py-2.5 border border-charcoal/10 text-charcoal/70 hover:bg-charcoal/5 rounded-xl text-sm font-bold transition">Cancel</a>
            <button type="submit" class="bg-gradient-to-r from-purple-500 to-indigo-600 hover:from-purple-600 hover:to-indigo-700 text-charcoal font-bold px-6 py-2.5 rounded-xl text-sm transition shadow-lg shadow-purple-500/20">Create Event</button>
        </div>
    </form>
</div>
@endsection
