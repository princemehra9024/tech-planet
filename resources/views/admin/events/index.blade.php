...
<div class="mb-8 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
    <div>
        ...
        <p class="text-muted">View and manage all official club events.</p>
    </div>
    @if(auth()->user()->role === 'admin')
    <a href="{{ route('admin.events.create') }}" class="bg-gradient-to-r from-purple-500 to-indigo-600 hover:from-purple-600 hover:to-indigo-700 text-charcoal font-bold px-6 py-2.5 rounded-xl shadow-lg shadow-purple-500/20 transition flex items-center gap-2">
...
<div class="space-y-4">
    @forelse($events as $event)
        <div class="glass-card rounded-2xl p-6 flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
            <div>
                <h3 class="text-xl font-bold text-charcoal font-display mb-1">{{ $event->title }}</h3>
                <p class="text-sm text-muted mb-3">{{ Str::limit($event->description, 100) }}</p>
                <div class="flex flex-wrap gap-4 text-xs font-semibold text-muted">
    ...
    @empty
        <div class="glass-card rounded-2xl p-12 text-center">
            <div class="w-16 h-16 bg-charcoal/5 rounded-full flex items-center justify-center mx-auto mb-4 border border-charcoal/10 ">
                <i class="fas fa-calendar-times text-2xl text-muted"></i>
            </div>
            <h3 class="text-xl font-bold text-charcoal mb-2">No Events Found</h3>
