...
@section('content')
<div class="mb-8">
    <a href="{{ route('admin.suggestions.index') }}" class="text-slate-400 hover:text-white transition flex items-center gap-2 mb-4 text-sm">
        <i class="fas fa-arrow-left"></i> Back to Suggestions
    </a>
    <h1 class="text-3xl font-display font-bold text-white mb-2">Publish Event</h1>
    <p class="text-slate-400">Finalize details to publish this suggestion as an official club event.</p>
</div>
...
<div class="glass-card rounded-2xl p-6 md:p-8 max-w-3xl">
    <form method="POST" action="{{ route('admin.suggestions.publish.store', $suggestion->id) }}" class="space-y-6">
        ...
        <div>
            ...
            <input type="text" name="title" required value="{{ old('title', $suggestion->title) }}" class="w-full bg-[#07080f]/60 border border-white/10 rounded-xl px-4 py-3 text-white focus:outline-none focus:ring-1 focus:ring-purple-500 transition">
        ...
        <div>
            ...
            <textarea name="description" required rows="5" class="w-full bg-[#07080f]/60 border border-white/10 rounded-xl px-4 py-3 text-white focus:outline-none focus:ring-1 focus:ring-purple-500 transition resize-y">{{ old('description', $suggestion->description) }}</textarea>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <input type="datetime-local" name="date" required value="{{ old('date', $suggestion->suggested_date ? \Carbon\Carbon::parse($suggestion->suggested_date)->format('Y-m-d\TH:i') : '') }}" class="w-full bg-[#07080f]/60 border border-white/10 rounded-xl px-4 py-3 text-white focus:outline-none focus:ring-1 focus:ring-purple-500 transition">
        ...
        <div class="pt-4 border-t border-white/5 flex justify-end gap-3">
            <a href="{{ route('admin.suggestions.index') }}" class="px-5 py-2.5 border border-white/10 text-slate-300 hover:bg-white/5 rounded-xl text-sm font-bold transition">Cancel</a>
            <button type="submit" class="bg-purple-600 hover:bg-purple-500 text-white font-bold px-6 py-2.5 rounded-xl text-sm transition shadow-lg shadow-purple-500/20">Publish Event</button>
        </div>
    </form>
</div>
@endsection
