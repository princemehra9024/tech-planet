...
@section('content')
<div class="mb-8">
    <h1 class="text-3xl font-display font-bold text-white mb-2">Event Suggestions & Votes</h1>
    <p class="text-slate-400">Review community suggestions and monitor voting activity.</p>
</div>
...
<div class="space-y-6">
        ...
        <div class="glass-card rounded-2xl p-6">
            <div class="flex flex-col md:flex-row justify-between items-start gap-4 mb-4">
                <div>
                    ...
                    <p class="text-sm text-slate-400">Suggested by <span class="text-white">{{ $suggestion->name }}</span> ({{ $suggestion->email }}) on {{ $suggestion->created_at->format('M d, Y') }}</p>
                ...
                <div class="flex gap-2">
                    @if($suggestion->status == 'pending')
                        <form action="{{ route('admin.suggestions.update-status', $suggestion->id) }}" method="POST">
                            ...
                            <button type="submit" class="bg-emerald-600 hover:bg-emerald-500 text-white px-4 py-2 rounded-xl text-sm font-bold transition">Approve for Voting</button>
                        </form>
                        <form action="{{ route('admin.suggestions.update-status', $suggestion->id) }}" method="POST">
                            @csrf
                            <input type="hidden" name="status" value="dismissed">
                            <button type="submit" class="bg-rose-600/50 hover:bg-rose-500 text-white px-4 py-2 rounded-xl text-sm font-bold transition border border-rose-500/30">Dismiss</button>
                        </form>
                    @elseif($suggestion->status == 'approved')
                        <div class="flex gap-2">
                            <a href="{{ route('admin.suggestions.publish', $suggestion->id) }}" class="bg-purple-600 hover:bg-purple-500 text-white px-4 py-2 rounded-xl text-sm font-bold transition shadow-lg shadow-purple-500/20">Publish Event</a>
                            <form action="{{ route('admin.suggestions.update-status', $suggestion->id) }}" method="POST">
                                @csrf
                                <input type="hidden" name="status" value="dismissed">
                                <button type="submit" class="bg-rose-600/50 hover:bg-rose-500 text-white px-4 py-2 rounded-xl text-sm font-bold transition border border-rose-500/30">Dismiss</button>
            ...
            <p class="text-slate-300 mb-6 bg-white/5 p-4 rounded-xl border border-white/10">{{ $suggestion->description }}</p>
            
            @if($suggestion->status == 'approved')
                <div class="mt-4 border-t border-white/10 pt-4">
                    <h4 class="text-sm font-bold text-white mb-3">Voting Results ({{ $totalVotes }} total votes)</h4>
    ...
    @if($suggestions->isEmpty())
        <div class="glass-card rounded-2xl p-12 text-center">
            ...
            <h3 class="text-xl font-bold text-white mb-2">No Suggestions Yet</h3>
            <p class="text-slate-400">When students submit event suggestions, they will appear here.</p>
        </div>
    @endif
</div>
@endsection
