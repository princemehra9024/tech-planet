...
<div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
    <div>
        <h1 class="text-3xl font-extrabold text-white font-display">Manage Posts</h1>
        <p class="text-sm text-slate-400 mt-1">All announcement posts created by admins.</p>
    </div>
    <a href="{{ route('admin.posts.create') }}" class="inline-flex items-center gap-1.5 bg-gradient-to-r from-cyan-500 to-purple-600 text-white font-bold px-6 py-2.5 rounded-xl text-sm shadow hover:shadow-lg transition">
...
<div class="space-y-4">
    ...
    <div class="glass-card rounded-2xl p-6 border border-white/5 shadow-md">
        <div class="flex items-start justify-between gap-4">
            <div>
                <p class="text-xs font-bold text-slate-500 uppercase tracking-wider">By {{ $post->user->name }} • {{ $post->created_at->diffForHumans() }}</p>
                <p class="mt-3 text-slate-200 text-sm leading-relaxed">{{ $post->content }}</p>
                @if($post->image_path)
